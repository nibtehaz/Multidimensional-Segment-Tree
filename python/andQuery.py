...
class Node(object):
	...
	def __init__(self, localValue=-1, globalValue=-1, localLazy=-1, globalLazy=-1):
...
class SegmentTree2D(object):
	...
	def queryByX(self, nodeID, xLo, xHi, qxLo, qxHi, qyLo, qyHi):
		...
		if (qxLo <= xLo and xHi <= qxHi):  # y is covered
			...
			return self.queryByY((nodeID[0], 1), xLo, xHi, 1, self.m, qxLo, qxHi, qyLo, qyHi, -1)
			...
			return self.queryByY(nodeID,xLo,xHi,1, self.m, qxLo, qxHi, qyLo, qyHi, -1) & self.queryByX(left, xLo, xMid, qxLo, qxHi, qyLo, qyHi) & self.queryByX(right, xMid+1, xHi, qxLo, qxHi, qyLo, qyHi)
